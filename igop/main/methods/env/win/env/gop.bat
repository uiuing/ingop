...
cd %USERPROFILE%\.igop\gop

go run cmd/make.go --build || "%USERPROFILE%\.igop\env\go\bin\go" run cmd/make.go --build

@echo off
set USERregpath=HKEY_CURRENT_USER\Environment

:: config
set igop_home=%USERPROFILE%\.igop
set GOPBIN=%igop_home%\gop\bin
