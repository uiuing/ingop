...
import { igopPaths, isWin } from '../config'
...
export function execCommand(cmd: string): AsyncStringNull {
  const e = join(igopPaths.env, 'env.bash')
...
export const existsEnv: ExistsEnv = {
  gop: {
    exist: async () => {
      ...
      let e = v !== ''
      if (!e) {
        v = await checkVersionEnv(join(igopPaths.gopBin, 'gop'))
        e = v !== ''
      }
      return { e, v }
    },
    ...
    isIGop: async () =>
      `"${await checkVersionEnv(join(igopPaths.gopBin, 'gop'))}"` !== ''
  },
  env: {
    go: {
      exist: async () => {
        ...
        let e = v !== ''
        if (!e) {
          v = await checkVersionEnv(join(igopPaths.goBin, 'go'))
          e = v !== ''
        }
        return { e, v }
      },
      ...
      isIGop: async () =>
        `"${await checkVersionEnv(join(igopPaths.gopBin, 'go'))}"` !== ''
    }
  }
}
